...
function updateUI() {
  ...
  if (coinResult) coinResult.textContent = currentArena.coinTossResult || "—";

  const canToss = currentArena.status === "occupied";
  tossBtn.disabled = !canToss;
  tossMessage.textContent = canToss ? "Il sorteggio e separato dal vincitore del match." : "La partita deve essere iniziata.";
}
...
function clearSelection() {
  coinLeft.classList.remove("selected");
  coinRight.classList.remove("selected");
}
...
function toss() {
  ...
  const interval = setInterval(() => {
    step += 1;
    if (step % 2 === 0) {
      coinLeft.classList.add("selected");
      coinRight.classList.remove("selected");
    } else {
      coinRight.classList.add("selected");
      coinLeft.classList.remove("selected");
    }
    if (step >= flashes) {
